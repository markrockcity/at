...
{
    bool MatchesUpTo(Limpl.IReadOnlyScanner<AtToken> tokens, int k);
    ExpressionSyntax ParseExpression(Limpl.IScanner<AtToken> tokens);
}

public class ExpressionRule : IExpressionRule
{
    readonly Func<Limpl.IReadOnlyScanner<AtToken>,int,bool> matchesUpTo;
    readonly Func<ExpressionRule,Limpl.IScanner<AtToken>,ExpressionSyntax> parse;  
    ...
    public ExpressionRule(Func<Limpl.IReadOnlyScanner<AtToken>,int,bool> matchesUpTo, Func<ExpressionRule,Limpl.IScanner<AtToken>,ExpressionSyntax> parse) 
    {  
        this.matchesUpTo = matchesUpTo;
        this.parse = parse;
    }

    public bool MatchesUpTo(Limpl.IReadOnlyScanner<AtToken> tokens,int k)=>matchesUpTo(tokens,k);
    public ExpressionSyntax ParseExpression(Limpl.IScanner<AtToken> input) => parse(this,input);
    ...
    {
        var s = new Limpl.Scanner<AtToken>(nodes.Cast<AtToken>());
        return ParseExpression(s);
    }

    static bool matchDirective(Limpl.IReadOnlyScanner<AtToken> s,int k)
    {
       return    k==0 && s.Current?.Kind==TokenKind.TokenCluster && s.Current?.Text?[0]=='#'
              || k==1 && s.LookAhead(1)?.Kind==TokenKind.TokenCluster;
    }

    static ExpressionSyntax directiveExpression(ExpressionRule rule,Limpl.IScanner<AtToken> tokens)
    {
...
public class ExpressionRuleList : ExpressionSourceList<IExpressionRule>
{
    ...
    public ExpressionRuleList Matches(Limpl.IReadOnlyScanner<AtToken> tokens, int k)
    {
